...
{
    std::cerr << "Generacion de codigo...n" << std::endl;
...
{
    std::cerr << "Creando integer: " << value << std::endl;
    context.code << value;
}

void NDouble::codeGen(CodeGenContext &context)
{
    std::cerr << "Creando double: " << value << std::endl;
    context.code << value;
}

void NString::codeGen(CodeGenContext &context)
{
    std::cerr << "Creando string: " << value << std::endl;
...
{
    std::cerr << "Creando char: " << value << std::endl;
...
void NIdentifier::codeGen(CodeGenContext &context)
{
    ...
    {
        context.errorMessage.push_back("Linea " + std::to_string(line) + ": Identificador \"" + name + "\" es indefinido");
    }
    else
    {
...
{
    std::cerr << "Creando method call: " << id.name << ", " << arguments.size() << " argumento " << std::endl;
    ...
    if ((id.name != "atoi") && (id.name != "strlen"))
    {
        ...
        if (bz)
        {
            context.errorMessage.push_back("Linea " + std::to_string(line) + ": Funcion \"" + id.name + "\" es indefinida");
            return;
        }
    }
...
{
    std::cerr << "Creando asignacion para " << lhs.name << std::endl;
...
{
    std::cerr << "Crando bloque" << std::endl;
    context.indent++;
    context.stack.push_back(std::vector<singleDeclaration>());
    std::cerr << "Identificadores actuales: " << context.indent << std::endl;

    StatementList::const_iterator it;
    for (it = statements.begin(); it != statements.end(); it++)
    {
        std::cerr << "Genreando codigo para " << typeid(**it).name() << std::endl;
    ...
    std::cerr << "Bloque de salida" << std::endl;
    context.indent--;
    std::cerr << "Identificadores actuales: " << context.indent << std::endl;
}
...
{
    std::cerr << "Generando codigo para " << typeid(expression).name() << std::endl;
...
void NVariableDeclaration::codeGen(CodeGenContext &context)
{
    ...
    {
        context.errorMessage.push_back("Line " + std::to_string(line) + ": Redefinicion de identificador \"" + id.name + "\"");
        return;
    }
    ...
    std::cerr << "Creando declaracion de variable " << type << " " << id.name << std::endl;
...
{
    std::cerr << "Creacion de funcion: " << id.name << std::endl;

    for (int i = 0; i < context.funcDeclaration.size(); i++)
    {
        if (context.funcDeclaration[i] == id.name)
        {
            context.errorMessage.push_back("Line " + std::to_string(line) + ": Redefinicion de funcion \"" + id.name + "\"");
            return;
        }
    }
...
{
    std::cerr << "Creacion de asignacion" << std::endl;
...
{
    std::cerr << "Creacion de declaracion" << std::endl;
...
{
    std::cerr << "creando declaracion de while" << std::endl;
...
{
    std::cerr << "Creacion declaracion return" << std::endl;
...
{
    std::cerr << "Creando declaracion array: " << id.name << std::endl;

    auto &layer = context.stack[context.stack.size() - 1];
    if (context.findLayerDeclaration(id.name))
    {
        context.errorMessage.push_back("Linea " + std::to_string(line) + ": Redefinicion de array \"" + id.name + "\"");
        return;
    }
    auto oldDeclaration = context.findDeclaration(id.name, 2);
    std::string newName = id.name;
...
{
    std::cerr << "Creacion de variable array" << std::endl;
